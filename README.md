...
* **FluentResults:** The application employs FluentResults for consistent error handling (RFC 7807) and result management, providing a clear and structured way to handle success and failure scenarios. In Api layer, TypedResults are used to return results with appropriate HTTP status codes.
...
* **Identity Management:** The application implements an account management system using ASP.NET Core Identity for user authentication and authorization, ensuring secure access to resources and personalized user experiences.
* **Identity Roles** Users can be assigned an Administrator role, unlocking advanced capabilities such as automated data seeding and comprehensive user management (Promote to Admin, Block, Warn, or Delete). The application guarantees data integrity by ensuring at least one active administrator account always remains in the system.
* **Activity Tracking** A custom middleware tracks the exact time of each user's last interaction with the system. It features a 5-minute cooldown mechanism to minimize database write operations and optimize resource utilization.
